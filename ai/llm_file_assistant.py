    ...
    messages = [
        {
            ...
            "content": (
                ...
                "do not call search_in_file per-file. Extract just the core topic word from the "
                "user's query, dropping generic trailing words like 'experience', 'skills', "
                "'background', 'knowledge' — e.g. for \"Find resumes mentioning Python "
                "experience\", search for 'python', NOT 'python experience' (the exact phrase "
                "won't appear verbatim in any file, only the core word will). Report every file "
                "the result actually contains — never skip one, never claim 'no others match' "
                "without checking the full result.\n\n"
